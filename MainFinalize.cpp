...
/**     ***********   BEGIN PRORAM OPTIONS   ************/
  ...
  try {
    ...
    config_file_options.add_options()
      ...
      ("cleanup-directory",
          po::value<std::string>(&cleanup_directory)->default_value("demac_cleanup"),
          "the directory where (finally) demacrofied files should be put to")
      ("backup-directory",
          po::value<std::string>(&backup_directory)->default_value("demac_backup"),
...
/*
    ...
    {
      if(no_translate){
        std::cerr<<"cleanup cannot proceed when no_translate is set to true.\n"
      ...
      if(cleanup){
        ...
        std::cout<<"Error! Set the cleanup to true in the config-file to finalize the translations\n";
        return 0;
      }
    }
